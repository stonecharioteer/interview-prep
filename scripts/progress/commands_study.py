...
def show_study_summary(repo_root: Path) -> None:
    ...
    if total_books:
        console.print(
            f"  Books: {in_progress_books or 0} in progress, {total_books} tracked"
        )

    total_courses, in_progress_courses, done_courses = courses
    if total_courses:
        console.print(
            f"  Courses: {done_courses or 0} completed, {in_progress_courses or 0} in progress, {total_courses} tracked"
        )

    total_papers, reading_papers, completed_papers = papers
    if total_papers:
        console.print(
            f"  Papers: {completed_papers or 0} read, {reading_papers or 0} reading, {total_papers} tracked"
        )

    if total_days > 0:
        console.print(
            f"  Streak: {current_streak} current, {longest_streak} best, {total_days} total days"
        )
...
def register_commands(study_app: typer.Typer) -> None:
    ...
    def book_add(
        ...
        chapters: Annotated[
            Optional[int], typer.Option("--chapters", "-c", help="Total chapters")
        ] = None,
    ...
    def book_read(
        ...
        chapter: Annotated[
            Optional[int], typer.Option("--chapter", "-c", help="Single chapter number")
        ] = None,
        chapters: Annotated[
            Optional[str],
            typer.Option("--chapters", help="Chapter range (e.g., '1-3' or '5,7')"),
        ] = None,
        progress: Annotated[
            int,
            typer.Option("--progress", "-p", help="Progress percentage (default: 100)"),
        ] = 100,
        date: Annotated[
            Optional[str],
            typer.Option("--date", "-d", help="Date (YYYY-MM-DD, default: today)"),
        ] = None,
    ):
        ...
        if book_id is None:
            ...
            if not books:
                console.print(
                    "[yellow]No books tracked yet. Use 'progress study book-add' to add one.[/yellow]"
                )
                conn.close()
                raise typer.Exit(1)
            book_id = prompt_select_from_list(
                [(b[0], b[1]) for b in books], "Select book"
            )
        ...
        if total_chapters:
            ...
            if invalid:
                console.print(
                    f"[red]Invalid chapters {invalid} (book has {total_chapters} chapters)[/red]"
                )
                conn.close()
                raise typer.Exit(1)
        ...
        if len(chapter_nums) == 1:
            ...
            console.print(
                f'[green]Updated:[/green] Chapter {ch} of "{book_title}" \u2192 {progress}%'
            )
        else:
            ch_list = ", ".join(str(c) for c in sorted(chapter_nums))
            console.print(
                f'[green]Updated:[/green] Chapters {ch_list} of "{book_title}" \u2192 {progress}%'
            )

    @study_app.command("book-list")
    def book_list(
        status: Annotated[
            Optional[StudyStatus],
    ...
    def book_show(
        ...
        if total_chapters and total_chapters > 0:
            ...
            total_progress = sum(
                progress_map.get(i, 0) for i in range(1, total_chapters + 1)
            )
        ...
        if total_chapters:
            console.print(
                f"  Chapters: {chapters_done}/{total_chapters} complete ({overall_pct}%)"
            )
        elif chapters_done:
    ...
    def course_add(
        ...
        source: Annotated[
            Optional[str],
            typer.Option("--source", "-s", help="Source (e.g., CMU, MIT)"),
        ] = None,
        code: Annotated[
            Optional[str],
            typer.Option("--code", "-c", help="Course code (e.g., 15-445)"),
        ] = None,
        url: Annotated[
            Optional[str], typer.Option("--url", help="Playlist URL")
        ] = None,
    ):
    ...
    def lecture_add(
        ...
        number: Annotated[
            ...
            typer.Option(
                "--number",
                "-n",
                help="Lecture number (auto-increments if not provided)",
            ),
        ] = None,
        ...
        if course_id is None:
            ...
            if not courses:
                console.print(
                    "[yellow]No courses tracked yet. Use 'progress study course-add' to add one.[/yellow]"
                )
                conn.close()
                raise typer.Exit(1)
            course_id = prompt_select_from_list(
                [(c[0], c[1]) for c in courses], "Select course"
            )
        ...
        result = conn.execute(
            "SELECT COALESCE(MAX(id), 0) + 1 FROM lectures"
        ).fetchone()
        new_id = result[0]
        ...
        console.print(
            f"[green]Added lecture #{number}:[/green] {title} (to {course[0]})"
        )

    @study_app.command("watch")
    def watch_lecture(
        course_id: Annotated[Optional[int], typer.Argument(help="Course ID")] = None,
        lecture_number: Annotated[
            Optional[int], typer.Argument(help="Lecture number")
        ] = None,
        status: Annotated[
            StudyStatus, typer.Option("--status", "-s", help="Status")
        ...
        if lecture_number is None:
            ...
            if not lectures:
                console.print(
                    f"[yellow]No lectures found for course #{course_id}.[/yellow]"
                )
                conn.close()
                raise typer.Exit(1)
            lecture_number = prompt_select_from_list(
                [(lecture_row[0], lecture_row[1]) for lecture_row in lectures],
                "Select lecture",
            )
        ...
        if not lecture:
            console.print(
                f"[red]Lecture #{lecture_number} not found in course #{course_id}[/red]"
            )
            conn.close()
            raise typer.Exit(1)
        ...
        console.print(
            f'[green]{status_word}:[/green] Lecture {lecture_number} "{lecture_title}" of {course_name}'
        )
    ...
    def course_show(
        ...
        if lectures:
            ...
            for num, title, url, status, date in lectures:
                status_sym = STUDY_STATUS_SYMBOL.get(
                    status or "not_started", STUDY_STATUS_SYMBOL["not_started"]
                )
            ...
            completed = sum(
                1 for lecture_row in lectures if lecture_row[3] == "completed"
            )
            total = len(lectures)
    ...
    def homework_add(
        ...
        url: Annotated[
            Optional[str], typer.Option("--url", help="Homework URL")
        ] = None,
    ):
        ...
        result = conn.execute(
            "SELECT COALESCE(MAX(id), 0) + 1 FROM homework"
        ).fetchone()
        new_id = result[0]
    ...
    def homework_done(
        homework_id: Annotated[
            Optional[int], typer.Argument(help="Homework ID")
        ] = None,
        date: Annotated[
            Optional[str],
            typer.Option("--date", "-d", help="Date (YYYY-MM-DD, default: today)"),
        ...
        if homework_id is None:
            ...
            homework_id = prompt_select_from_list(
                [(h[0], h[1]) for h in homework], "Select homework"
            )

        hw = conn.execute(
            "SELECT name FROM homework WHERE id = ?", [homework_id]
        ).fetchone()
        if not hw:
    ...
    def project_done(
        ...
        if project_id is None:
            ...
            project_id = prompt_select_from_list(
                [(p[0], p[1]) for p in projects], "Select project"
            )
    ...
    def paper_add(
        ...
        authors: Annotated[
            Optional[str], typer.Option("--authors", "-a", help="Authors")
        ] = None,
        url: Annotated[Optional[str], typer.Option("--url", help="Paper URL")] = None,
        year: Annotated[
            Optional[int], typer.Option("--year", "-y", help="Publication year")
        ] = None,
    ):
    ...
    def paper_read(
        ...
        if paper_id is None:
            ...
            paper_id = prompt_select_from_list(
                [(p[0], p[1]) for p in papers], "Select paper"
            )
    ...
    def paper_list(
        ...
        if not papers:
            console.print(
                "[yellow]No papers tracked yet. Use 'progress study paper-add' to add one.[/yellow]"
            )
            return
        ...
        for paper_id, title, authors, year, paper_status, date in papers:
            ...
            status_sym = STUDY_STATUS_SYMBOL.get(
                paper_status, STUDY_STATUS_SYMBOL["not_started"]
            )
    ...
    def lecture_chapters(
        ...
        book_id: Annotated[Optional[int], typer.Argument(help="Book ID")] = None,
        chapters: Annotated[
            Optional[str], typer.Argument(help="Chapters (e.g., '1-3' or '5,7')")
        ] = None,
    ):
        ...
        console.print(
            f'[green]Associated:[/green] Chapters {chapters} of "{book[0]}" with "{lecture[0]}"'
        )
    ...
    def study_recent(
        ...
        if not activities:
            console.print(
                f"[yellow]No study activity in the last {days} days.[/yellow]"
            )
            return
